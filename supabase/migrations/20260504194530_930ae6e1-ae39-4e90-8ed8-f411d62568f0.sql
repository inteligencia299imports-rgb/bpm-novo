...
AS $function$
DECLARE v_loja text := lower(trim(coalesce(_loja, ''))); v_result json;
BEGIN
  IF v_loja IN ('', 'todos') THEN v_loja := NULL; END IF;
  WITH base AS (
    SELECT av.id, av.tipo_aquisicao, av.situacao, av.created_at, av.updated_at, a.interesse,
      COALESCE((SELECT MIN(sh.created_at) FROM status_history sh WHERE sh.entity_id = av.id AND sh.entity_type = 'avaliacao' AND sh.status = 'adquirida'), av.updated_at, av.created_at) AS data_aquisicao
    FROM avaliacoes av JOIN atendimentos a ON a.id = av.atendimento_id
  ...
  ), filtrado_avaliacoes AS (SELECT * FROM base WHERE (_date_from IS NULL OR created_at >= _date_from) AND (_date_to IS NULL OR created_at <= _date_to)),
  filtrado_aquisicoes AS (SELECT * FROM base WHERE tipo_aquisicao IN ('propria','consignada','test-ride','repasse','convertida') AND (_date_from IS NULL OR data_aquisicao >= _date_from) AND (_date_to IS NULL OR data_aquisicao <= _date_to))
  SELECT json_build_object('total_avaliacoes', (SELECT COUNT(*) FROM filtrado_avaliacoes), 'total_aquisicoes', (SELECT COUNT(*) FROM filtrado_aquisicoes), 'aquisicoes_propria', (SELECT COUNT(*) FROM filtrado_aquisicoes WHERE tipo_aquisicao IN ('propria','convertida','repasse','test-ride')), 'aquisicoes_consignada', (SELECT COUNT(*) FROM filtrado_aquisicoes WHERE tipo_aquisicao = 'consignada'), 'aquisicoes_convertida', (SELECT COUNT(*) FROM filtrado_aquisicoes WHERE tipo_aquisicao = 'convertida'), 'entrada_direta', (SELECT COUNT(*) FROM filtrado_aquisicoes WHERE interesse = 'vender'), 'troca', (SELECT COUNT(*) FROM filtrado_aquisicoes WHERE interesse = 'trocar'), 'retiradas', (SELECT COUNT(*) FROM filtrado_avaliacoes WHERE situacao = 'retirada')) INTO v_result;
  RETURN v_result;
END;
$function$;
...
AS $function$
DECLARE result jsonb := '[]'::jsonb; v_start date := '2025-12-21'; v_now date := current_date; v_cycle_start timestamptz; v_cycle_end timestamptz; v_label text; v_loja text := lower(trim(coalesce(_loja, 'todos')));
BEGIN
  IF v_loja IN ('', 'todos') THEN v_loja := 'todos'; END IF;
...
BEGIN
  ...
  WHERE a.situacao = 'vendido' AND (_loja = 'todos' OR norm_loja(a.loja) = _loja OR a.loja = _loja)
    AND COALESCE(e.data_venda, a.data_venda) IS NOT NULL AND (_tipo = 'todos' OR COALESCE(e.tipo, 'propria') = _tipo)
    AND (_date_from IS NULL OR COALESCE(e.data_venda, a.data_venda) >= _date_from) AND (_date_to IS NULL OR COALESCE(e.data_venda, a.data_venda) <= _date_to);
...
BEGIN
  WHILE v_start <= v_now LOOP
    ...
    BEGIN
      ...
      SELECT count(*) INTO v_vendas FROM atendimentos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND COALESCE(e.data_venda,a.data_venda) IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND COALESCE(e.data_venda,a.data_venda) >= v_cycle_start AND COALESCE(e.data_venda,a.data_venda) <= v_cycle_end;
      FOR rec IN SELECT a.id as atend_id, a.valor_venda as atend_valor_venda, e.preco, e.valor_venda as estoque_valor_venda, av.id as avaliacao_id, av.quanto_vende, av.valor_fechamento
        FROM atendimentos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id LEFT JOIN avaliacoes av ON av.id = e.avaliacao_id
        WHERE a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND COALESCE(e.data_venda,a.data_venda) IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND COALESCE(e.data_venda,a.data_venda) >= v_cycle_start AND COALESCE(e.data_venda,a.data_venda) <= v_cycle_end
      LOOP
...
BEGIN
  FOR v IN SELECT ur.user_id, ur.nome FROM user_roles ur LOOP
    ...
    BEGIN
      ...
      SELECT count(*) INTO v_vendas FROM atendimentos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.vendedor_id=v.user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND COALESCE(e.data_venda,a.data_venda) IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND (_date_from IS NULL OR COALESCE(e.data_venda,a.data_venda) >= _date_from) AND (_date_to IS NULL OR COALESCE(e.data_venda,a.data_venda) <= _date_to);
      ...
      SELECT COALESCE(SUM(COALESCE(e.preco, a.valor_venda, 0)),0) INTO v_faturamento FROM atendimentos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.vendedor_id=v.user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND COALESCE(e.data_venda,a.data_venda) IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND (_date_from IS NULL OR COALESCE(e.data_venda,a.data_venda) >= _date_from) AND (_date_to IS NULL OR COALESCE(e.data_venda,a.data_venda) <= _date_to);
...
BEGIN
  FOR rec IN
    SELECT a.nome_cliente, a.loja, ur.nome as vendedor_nome,
      COALESCE(e.tipo, CASE WHEN norm_loja(a.loja)='Ducati' THEN 'ducati' ELSE 'propria' END) as tipo,
      COALESCE(e.marca || ' ' || e.modelo, mi.marca || ' ' || mi.modelo, '-') as modelo,
      COALESCE(e.placa,'-') as placa, COALESCE(e.data_venda, a.data_venda) as data_venda,
...
BEGIN
  ...
  SELECT count(*) INTO v_vendas FROM atendimentos a LEFT JOIN estoque e ON e.atendimento_venda_id=a.id WHERE a.vendedor_id=_user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND COALESCE(e.data_venda,a.data_venda) IS NOT NULL AND (_date_from IS NULL OR COALESCE(e.data_venda,a.data_venda) >= _date_from) AND (_date_to IS NULL OR COALESCE(e.data_venda,a.data_venda) <= _date_to);
