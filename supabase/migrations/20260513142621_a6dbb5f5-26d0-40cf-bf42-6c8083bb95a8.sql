...
BEGIN
  WHILE v_start <= v_now LOOP
    v_cycle_start := (v_start::timestamp) AT TIME ZONE 'America/Sao_Paulo';
    v_cycle_end := (((v_start + interval '1 month' - interval '1 day')::date::timestamp + interval '23 hours 59 minutes 59 seconds 999 milliseconds') AT TIME ZONE 'America/Sao_Paulo');
    ...
    BEGIN
      ...
      WHERE a.situacao = 'vendido'
        ...
        AND a.data_venda >= v_cycle_start
        AND a.data_venda <= v_cycle_end;
      ...
      LOOP
        ...
        BEGIN
          ...
          WHERE cc.atendimento_id = rec.atend_id
            AND lower(co.responsavel) = 'loja';
