...
-- 1) Add loja (pátio) column to estoque
ALTER TABLE public.estoque ADD COLUMN IF NOT EXISTS loja text;

-- 2) Backfill from atendimentos via avaliacao_id where possible
UPDATE public.estoque e
SET loja = a.loja
FROM public.avaliacoes av
JOIN public.atendimentos a ON a.id = av.atendimento_id
WHERE e.avaliacao_id = av.id
  AND e.loja IS NULL
  AND a.loja IS NOT NULL;

-- 3) Update KPIs function to filter by loja column with group logic
CREATE OR REPLACE FUNCTION public.relatorio_estoque_kpis(
  p_cutoff timestamp with time zone DEFAULT now(),
  p_loja text DEFAULT 'todos'::text,
  p_tipo text DEFAULT 'todos'::text
)
RETURNS jsonb
LANGUAGE plpgsql
  ...
  RETURN (
    ...
    prep AS (
      ...
      WHERE av.situacao IN ('adquirida','estoque')
        ...
        AND (
          v_loja = 'todos'
          OR (v_loja = '299' AND upper(coalesce(a.loja,'')) NOT LIKE '%DUCATI%')
          OR (v_loja = 'Ducati' AND upper(coalesce(a.loja,'')) LIKE '%DUCATI%')
          OR lower(coalesce(a.loja,'')) = lower(v_loja)
        )
        AND (
          v_tipo = 'todos'
...
-- 4) Update mensal function similarly
...
BEGIN
  WHILE v_start <= v_cutoff::date LOOP
    ...
    BEGIN
      ...
      SELECT count(*) INTO v_disponiveis FROM estoque
      ...
      SELECT COALESCE(SUM(preco), 0) INTO v_patrimonio FROM estoque
       WHERE data_entrada <= v_ce AND (data_venda IS NULL OR data_venda > v_ce)
         AND (v_loja='todos'
              OR (v_loja='299' AND (upper(coalesce(loja,'')) NOT LIKE '%DUCATI%' OR coalesce(loja,'')=''))
              OR (v_loja='Ducati' AND upper(coalesce(loja,'')) LIKE '%DUCATI%')
