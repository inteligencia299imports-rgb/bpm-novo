    ...
    WITH active AS (
      ...
      WHERE status IN ('disponivel','indisponivel','servico','bloqueio_juridico')
        ...
        AND (
          ...
          OR (v_loja IN ('Brasília','Brasilia') AND coalesce(loja,'') IN ('299i','299s','Aventura','Ducati BSB'))
          OR (v_loja = 'Florianópolis' AND coalesce(loja,'') IN ('299f','Ducati FLN'))
          OR (v_loja = 'Porto Alegre' AND coalesce(loja,'') IN ('299p','Ducati POA'))
    ...
    prep AS (
      ...
      WHERE av.situacao IN ('adquirida','estoque')
        ...
        AND (
          ...
          OR (v_loja IN ('Brasília','Brasilia') AND coalesce(a.loja,'') IN ('299i','299s','Aventura','Ducati BSB'))
          OR (v_loja = 'Florianópolis' AND coalesce(a.loja,'') IN ('299f','Ducati FLN'))
          OR (v_loja = 'Porto Alegre' AND coalesce(a.loja,'') IN ('299p','Ducati POA'))
