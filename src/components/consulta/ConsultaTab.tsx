...
const ConsultaTab = () => {
  ...
  const fetchMotos = useCallback(async () => {
    ...
    const { data, error } = await fetchAllRange(() =>
      supabase
        .from('avaliacoes')
        .select(`*, ${MARCA_MODELO_SELECT}, atendimentos_motos!inner(id, loja_id, loja_empresas:loja_id(loja), cliente_id, cliente:clientes_fornecedores(nome_razao_social, telefone, tipo_pessoa, data_nascimento, cpf_cnpj, email, clientes_fornecedores_enderecos(cep, logradouro)))`)
