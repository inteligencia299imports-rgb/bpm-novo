...
function drawJustifiedText(doc: jsPDF, text: string, x: number, maxWidth: number, y: number, lineHeight: number, undefined, lineCheckPageBreak): number {
...
export async function generateContratoCompraPdf(data: ContratoCompraPdfData): Promise<void> {
  ...
  y = drawJustifiedText(doc, 'Citado neste documento no campo "Vendedor", e assina o campo "Assinatura do cliente" ao final deste documento.', marginLeft, contentWidth, y, lineHeight, lineCheckPageBreak);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(lineHeight);
  y = drawJustifiedText(doc, 'O COMPRADOR declara ter vistoriado e avaliado o estado em que se encontra o veículo ora negociado, estando o mesmo em perfeitas condições de funcionamento e estado de conservação.', marginLeft, contentWidth, y, lineHeight, lineCheckPageBreak);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(lineHeight);
  y = drawJustifiedText(doc, 'Nos termos do que estabelece o artigo 629 do Código Civil, o COMPRADOR assume, de forma gratuita, a condição de depositário do bem objeto do presente, obrigando-se pela guarda e conservação do mesmo, até o integral pagamento do preço.', marginLeft, contentWidth, y, lineHeight, lineCheckPageBreak);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(lineHeight);
  y = drawJustifiedText(doc, 'O comprador está totalmente ciente que está adquirindo uma motocicleta semi nova e está de acordo em adquirir a motocicleta com quaisquer detalhes, arranhados e demais avarias estéticas que a moto tenha, mediante avaliação prévia antes da negociação e assinatura deste.', marginLeft, contentWidth, y, lineHeight, lineCheckPageBreak);
  y += sectionGap;
