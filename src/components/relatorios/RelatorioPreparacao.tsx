...
const RelatorioPreparacao: React.FC<Props> = ({ dateFrom, dateTo, setDateFrom, setDateTo, onRegisterClear, onFilterChange }) => {
  ...
  const kpis = useMemo(() => {
    ...
    // Em preparação agora: alinhado ao kanban (em_aberto, pendente, oficina, servico_externo)
    const kanbanStatuses = ['em_aberto', 'pendente', 'oficina', 'servico_externo'];
    const emPreparacao = rows.filter(r =>
      matchesLoja(r.loja, filterLoja) &&
      (filterTipo === 'todos' || r.tipoCat === filterTipo) &&
      r.situacao !== 'perdido' &&
      !r.dataLiberacao &&
      kanbanStatuses.includes(r.statusPrep)
    ).length;
    return {
      emPreparacao,
  ...
  return (
    <div className="space-y-4 w-full max-w-full overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
        <KpiCard title="Em Preparação" value={kpis.emPreparacao} icon={<Wrench className="h-5 w-5" />} color="orange" />
        <KpiCard title="Motos Preparadas" value={kpis.qtdPreparadas} icon={<CheckCircle className="h-5 w-5" />} color="teal" />
