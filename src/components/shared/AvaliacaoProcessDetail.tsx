...
const AvaliacaoProcessDetail: React.FC<Props> = ({ item, entityType, statusColumns, statusField, title, onClose }) => {
  ...
  const handleSaveMoto = async () => {
    ...
    if (crlvUrl) {
      ...
      delete motoUpdate.marca_id;
      delete motoUpdate.modelo_id;
    ...
    if (error) {
      ...
      const marcaNome = marcasCatalogo.find(x => x.id === editMarcaId)?.nome ?? moto.marca ?? '';
      const modeloNome = getModelosByMarcaId(editMarcaId).find(x => x.id === editModeloId)?.nome ?? moto.modelo ?? '';
      setMotoData({ ...moto, ...motoUpdate, marca_id: editMarcaId, modelo_id: editModeloId, marca: marcaNome, modelo: modeloNome });
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <DocumentUpload
                        ...
                        onUploaded={async (url) => {
                          setAtpvUrl(url);
                          if (!moto.id) return;
                          await supabase.from('avaliacoes').update({ atpv_url: url }).eq('id', moto.id);
                          const { ok } = await conferirDocVeiculo(moto.id, url, 'atpv');
                          if (!ok) {
                            await supabase.from('avaliacoes').update({ atpv_url: null }).eq('id', moto.id);
                            setAtpvUrl(null);
                            await removerDocDoStorage(`docs/${moto.id}/atpv`);
                          }
                        }}
                        onRemoved={() => {
                          setAtpvUrl(null);
                      ...
                      <DocumentUpload
                        ...
                        onUploaded={async (url) => {
                          ...
                          if (!moto.id) return;
                          await supabase.from('avaliacoes').update({ procuracao_url: url }).eq('id', moto.id);
                          const { ok } = await conferirDocVeiculo(moto.id, url, 'procuracao');
                          if (!ok) {
                            await supabase.from('avaliacoes').update({ procuracao_url: null }).eq('id', moto.id);
                            setProcuracaoUrl(null);
                            await removerDocDoStorage(`docs/${moto.id}/procuracao`);
                          }
                        }}
                        onRemoved={() => {
                          setProcuracaoUrl(null);
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-6 py-2 pr-7" style={{ scrollbarWidth: 'thin' }}>
            <div className="space-y-3 pb-4">
              ...
              <div>
                ...
                <Select value={editMarcaId} onValueChange={(v) => { setEditMarcaId(v); setEditModeloId(''); }} disabled={!!crlvUrl}>
              ...
              <div>
                ...
                <Select value={editModeloId} onValueChange={setEditModeloId} disabled={!!crlvUrl || !editMarcaId}>
                  <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
