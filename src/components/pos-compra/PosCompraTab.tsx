...
const PosCompraTab = ({ initialAvaliacaoId, onInitialHandled }: PosCompraTabProps = {}) => {
  ...
  useEffect(() => {
    if (initialAvaliacaoId) {
      supabase.from('avaliacoes')
        .select(`*, ${MARCA_MODELO_SELECT}, atendimentos_motos!avaliacoes_atendimento_id_fkey!inner(id, loja_id, loja_empresas:loja_id(loja), cliente_id, interesse, cliente:clientes_fornecedores(nome_razao_social, telefone, tipo_pessoa, cpf_cnpj, email, clientes_fornecedores_enderecos(cep, logradouro)))`)
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    const selectStr = `*, ${MARCA_MODELO_SELECT}, atendimentos_motos!avaliacoes_atendimento_id_fkey!inner(id, loja_id, loja_empresas:loja_id(loja), cliente_id, interesse, cliente:clientes_fornecedores(nome_razao_social, telefone, tipo_pessoa, cpf_cnpj, email, clientes_fornecedores_enderecos(cep, logradouro)))`;
    ...
    if (error) { toast.error('Erro ao carregar pós-compra'); } else {
      ...
      const histEntityIds = Array.from(new Set(avalIds));
      ...
      (data || []).forEach((d: any) => {
        ...
        const picked = [byAval].filter(Boolean).sort((a, b) => new Date(b!).getTime() - new Date(a!).getTime())[0]
