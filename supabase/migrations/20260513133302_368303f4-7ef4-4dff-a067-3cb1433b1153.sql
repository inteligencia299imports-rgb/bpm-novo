...
DECLARE
  ...
  v_cycle_next timestamptz;
  v_label text;
BEGIN
  WHILE v_start <= v_now LOOP
    v_cycle_start := (v_start::timestamp) AT TIME ZONE 'America/Sao_Paulo';
    v_cycle_next := ((v_start + interval '1 month')::date::timestamp) AT TIME ZONE 'America/Sao_Paulo';
