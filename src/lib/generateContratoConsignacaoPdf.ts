...
export async function generateContratoConsignacaoPdf(data: ContratoConsignacaoPdfData): Promise<void> {
  ...
  checkPageBreak(30);
  y = drawJustifiedText(doc, consignatariaText, marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  if (data.comPercentual5) {
    ...
    y = drawJustifiedText(doc, pagText, marginLeft, contentWidth, y, lineHeight);
  } else {
  ...
  setNormal();
  checkPageBreak(15);
  y = drawJustifiedText(doc, '§1º A transferência do valor será realizada, preferencialmente, por meio de depósito ou transferência bancária para a conta indicada pelo(a) CONSIGNANTE, após a entrega de toda a documentação solicitada.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(15);
  y = drawJustifiedText(doc, '§2º Após a concretização da venda, a CONSIGNATÁRIA se compromete a prestar o suporte necessário ao comprador para a efetivação da transferência do veículo, auxiliando-o no cumprimento dos trâmites administrativos e burocráticos junto aos órgãos competentes.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  checkPageBreak(25);
  if (data.comPercentual5) {
    y = drawJustifiedText(doc, 'Fica desde já convencionado entre as partes que, na hipótese de o veículo objeto do presente contrato vir a ser alienado, seja pelo valor indicado no campo "VALOR" ou por quantia inferior, desde que haja anuência expressa do(a) CONSIGNANTE —, será devida à CONSIGNATÁRIA comissão correspondente a 5% (cinco por cento) do valor efetivo da transação. Tal comissão será automaticamente retida pela CONSIGNATÁRIA no ato do pagamento efetuado pelo comprador, por ocasião da quitação final.', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§1º Caso o(a) CONSIGNANTE deseje fixar o valor líquido que pretende auferir com a venda, deverá informar tal quantia de maneira clara e expressa no momento da consignação, a fim de que conste no campo "Observações" do presente contrato.', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§2º Os valores previstos nesta cláusula, inclusive a comissão ora estipulada, permanecerão devidos à CONSIGNATÁRIA mesmo que a venda do veículo se concretize após a devolução do bem ao(à) CONSIGNANTE, desde que o(a) comprador(a) tenha sido por ela apresentado(a) ao(à) CONSIGNANTE, de forma direta ou indireta, durante a vigência deste instrumento.', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§3º O(a) CONSIGNANTE declara ter pleno conhecimento de que, enquanto vigente o presente contrato, não poderá dispor do veículo para venda direta a terceiros. Qualquer divulgação do bem por iniciativa própria, inclusive por meio de plataformas de venda online ou quaisquer outros meios, será considerada infração contratual, caracterizando violação às obrigações aqui assumidas.', marginLeft, contentWidth, y, lineHeight);
  } else {
    y = drawJustifiedText(doc, 'Fica desde já estabelecido, que independentemente do valor de venda, será repassado ao consignante o valor estipulado no campo "VALOR", ao proprietário do veículo.', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§1º Caso o CONSIGNANTE estabeleça o valor líquido pretendido com a venda, este deverá informar de forma clara no momento da consignação para ser especificado no campo "Observações".', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§2° Os valores referidos nesta cláusula, persistirão em favor da CONSIGNATÁRIA, acaso o veículo objeto desta seja negociado após sua devolução ao CONSIGNANTE, diretamente por este ou por outrem e o comprador seja, justamente, pessoa que tenha sido apresentada, na vigência deste, ao CONSIGNANTE, pela CONSIGNATÁRIA;', marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
    checkPageBreak(15);
    y = drawJustifiedText(doc, '§3° O CONSIGNANTE fica ciente que durante a vigência deste contrato, não poderá dispor do objeto para venda direta e o ato contrário, como a publicação em canais de venda, se caracteriza como quebra deste contrato.', marginLeft, contentWidth, y, lineHeight);
  }
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(15);
  y = drawJustifiedText(doc, 'Todas as imagens produzidas na transação de consignação são de direito exclusivo da CONSIGNATÁRIA.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  y = drawJustifiedText(doc, '§1° As imagens produzidas pela CONSIGNATÁRIA serão de uso e direito para esta comercialização, vetado o repasse ou uso sem permissão, sob pena de impor despesas evasivas.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§2° O CONSIGNANTE fica ciente que durante a vigência deste contrato não poderá dispor do objeto para anúncios particulares e venda direta. Sob pena de eventuais cobranças judiciais.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§3° O CONSIGNANTE DEVE RETIRAR IMEDIATAMENTE QUALQUER ANÚNCIO REFERENTE A MOTO, OBJETO DESTE CONTRATO, DE CANAIS DE VENDAS DIVERSOS, E NÃO PODERÁ FAZER NOVOS ANÚNCIOS ENQUANTO A MOTO ESTIVER CONSIGNADA NA LOJA.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(35);
  y = drawJustifiedText(doc, 'O CONSIGNANTE se responsabiliza pela evicção, por eventuais vícios redibitórios, bem como todas as multas, sejam elas Federais, Estaduais e/ou Municipais, até a presente data. Diante disso, esta CONSIGNATÁRIA fica desde já autorizada, após tentativa amigável de cobrança, mediante notificação extrajudicial, a qual deverá ser respaldada nos documentos de origem dos débitos, a proceder à execução do valor atualizado do débito, devidamente acrescido da multa eventualmente impingida pelos órgãos públicos, mais juros de mora de 1% ao mês, a contar da data em que a CONSIGNATÁRIA, ou alguém por ela despenda tais valores. Deste modo, o (a) CONSIGNANTE declara que reconhece que o presente instrumento é firmado no termo do artigo 585, II do CPC, razão pela qual o presente é um título executivo extrajudicial, mesmo porque, o "quantum debeatur" depende de simples cálculo aritmético, a partir de dados consignados em documentos comprobatórios do débito (multas de trânsito, IPVA e outros). Nesta seara, a CONSIGNATÁRIA poderá executar o presente para cobrar os valores eventualmente devidos e de responsabilidade do (a) CONSIGNANTE. Para dirimir quaisquer dúvidas decorrentes do presente, as partes estabelecem desde já o foro da comarca de Brasília, por mais privilegiado que outro possa ser, sendo que o CONSIGNANTE RENUNCIA ao foro previsto no inciso I, do artigo 101, do Código de Defesa do Consumidor. O veículo objeto do presente ficará sob guarda e responsabilidade da CONSIGNATÁRIA, até o dia da venda ou sua devolução ao CONSIGNANTE;', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§1° O CONSIGNANTE declara que o veículo acima descrito encontra-se desobstruído para transferência em todo TERRITÓRIO NACIONAL. Sendo de sua responsabilidade multas e débitos até a presente data.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§2° O CONSIGNANTE que optou pela ISENÇÃO DE IPVA, no momento da compra do veículo acima descrito, deverá arcar com os débitos em aberto junto à Secretaria de Fazenda do estado de origem. Visando a transferência de propriedade após a conclusão da venda.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§3° Fica desde já estabelecido que, o início da contagem do prazo de responsabilidade, do consignante por eventuais defeitos e vícios ocultos se dará após a concretização da venda, com a assinatura e reconhecimento do CRV (DUT).', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§4° Caso o CONSIGNANTE precise se ausentar do Distrito Federal, deverá deixar uma procuração para alguém de sua confiança, autorizando essa pessoa a agir em seu nome.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  for (const obr of obrigacoes) {
    checkPageBreak(15);
    y = drawJustifiedText(doc, obr, marginLeft, contentWidth, y, lineHeight);
    y += sectionGap;
  }
  ...
  setNormal();
  checkPageBreak(15);
  y = drawJustifiedText(doc, 'A previsão legal dos Vícios Redibitórios está contida no art. 441 do Código Civil, sendo que esses Vícios são DEFEITOS OCULTOS em coisa recebida em virtude de contrato comutativo, que a tornem imprópria para o uso ou lhe diminuam o valor.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;

  // CONTRATO DE CONSIGNAÇÃO (continuation header)
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§1° O CONSIGNANTE é responsável pelo objeto deste contrato e pelos seus defeitos, ainda que os desconheça até o momento.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  checkPageBreak(15);
  y = drawJustifiedText(doc, '§2° O check list não exime o CONSIGNANTE de sua responsabilidade em relação ao objeto. A 299 Imports realizará um laudo técnico de verificação da motocicleta após a assinatura deste contrato. Este laudo é caracterizado conforme abaixo:', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  // Scanner text
  checkPageBreak(20);
  y = drawJustifiedText(doc, 'O veículo passará por nosso scanner para uma verificação completa (laudo de entrada e também será repetido na saída) - O relatório será anexado ao processo e histórico da moto. O scanner verifica:', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(10);
  y = drawJustifiedText(doc, '§1° Fica ajustado que as comunicações/notificações que tratam o presente contrato poderão ser encaminhadas da seguinte forma:', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
  ...
  setNormal();
  checkPageBreak(15);
  y = drawJustifiedText(doc, 'Para dirimir quaisquer dúvidas decorrentes do presente, as partes estabelecem desde já, com exclusividade, o foro da comarca de Brasília–DF, por mais privilegiado que outro possa ser. O comprador de livre e espontânea vontade renuncia ao foro previsto no artigo 101, I, do Código de Defesa do Consumidor.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;

  // LGPD + Digital
  checkPageBreak(20);
  y = drawJustifiedText(doc, 'Conforme a Lei Geral de Proteção de Dados (LGPD), Lei n.º 13.709/2018, o cliente consente expressamente com a utilização dos seus dados pessoais, fornecidos neste contrato a fins de contato e comunicação comercial pela empresa.', marginLeft, contentWidth, y, lineHeight);
  y += sectionGap;
