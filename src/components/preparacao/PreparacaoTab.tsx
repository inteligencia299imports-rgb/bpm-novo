...
const PreparacaoTab = ({ initialAvaliacaoId, onInitialHandled }: PreparacaoTabProps = {}) => {
  ...
  useEffect(() => {
    if (initialAvaliacaoId) {
      supabase.from('avaliacoes')
        .select(`*, ${MARCA_MODELO_SELECT}, atendimentos_motos!avaliacoes_atendimento_id_fkey!inner(id, loja_id, loja_empresas:loja_id(loja), cliente_id, cliente:clientes_fornecedores(nome_razao_social, telefone, tipo_pessoa, cpf_cnpj, email, clientes_fornecedores_enderecos(cep, logradouro)))`)
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    const selectStr = `*, ${MARCA_MODELO_SELECT}, atendimentos_motos!avaliacoes_atendimento_id_fkey!inner(id, loja_id, loja_empresas:loja_id(loja), cliente_id, cliente:clientes_fornecedores(nome_razao_social, telefone, tipo_pessoa, cpf_cnpj, email, clientes_fornecedores_enderecos(cep, logradouro)))`;
    ...
    if (err1) { toast.error('Erro ao carregar preparação'); } else {
      ...
      let mapped = allData
        // Motos próprias só entram na Preparação depois de aprovadas no Pós-Compra.
        .filter((d: any) => !(isTipoPropria(d.tipo_aquisicao) && (d.aprovacao_status === 'aguardando' || d.aprovacao_status === 'recusada')))
        .map((d: any) => ({ ...d, atendimento: { ...d.atendimentos_motos, loja: d.atendimentos_motos?.loja_empresas?.loja }, moto: d, _estoqueInfo: estoqueMap[d.id] || null, _releaseReady: releaseReadyMap[d.id] ?? null, _dataAquisicao: acquDateMap[d.id] || null }));
                ...
                <div key={col.value} className="w-[260px] shrink-0 md:w-auto md:shrink flex flex-col">
                  ...
                  <div className="bg-muted/50 rounded-lg p-2.5 flex-1 min-h-[200px] space-y-2.5 border border-border/50">
                    {colItems.length === 0 ? <p className="text-xs text-muted-foreground text-center py-8">Nenhum item</p> : colItems.map((a: any) => (
                      <ProcessCard key={a.id} clientName={a.atendimento?.cliente?.nome_razao_social || 'N/A'}
