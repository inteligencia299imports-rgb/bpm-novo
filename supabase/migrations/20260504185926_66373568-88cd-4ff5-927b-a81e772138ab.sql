...
BEGIN
  FOR rec IN
    SELECT
      ...
      av.valor_fechamento,
      av.avaliacao_compra,
      av.avaliacao_consignacao,
      c.valor_fechamento as contrato_valor_fechamento,
      cc.valor_fechamento as consignante_valor_fechamento,
      a.valor_venda as atend_valor_venda,
      e.valor_venda as estoque_valor_venda,
      e.preco as estoque_preco,
      e.preco_acao as estoque_preco_acao,
      av.id as avaliacao_id,
      a.id as atendimento_id
    FROM atendimentos a
    LEFT JOIN LATERAL (
      SELECT mi2.marca, mi2.modelo, mi2.estoque_moto_id
      FROM motos_interesse mi2
      WHERE mi2.atendimento_id = a.id
      ORDER BY (mi2.estoque_moto_id IS NOT NULL) DESC, mi2.created_at ASC
      LIMIT 1
    ) mi ON true
    LEFT JOIN LATERAL (
      SELECT e2.*
      FROM estoque e2
      WHERE e2.atendimento_venda_id = a.id
         OR (mi.estoque_moto_id IS NOT NULL AND e2.id::text = mi.estoque_moto_id)
      ORDER BY
        CASE
          WHEN mi.estoque_moto_id IS NOT NULL AND e2.id::text = mi.estoque_moto_id THEN 0
          WHEN e2.atendimento_venda_id = a.id THEN 1
          ELSE 2
        END,
        e2.updated_at DESC NULLS LAST,
        e2.created_at DESC NULLS LAST
      LIMIT 1
    ) e ON true
    LEFT JOIN LATERAL (
      SELECT av2.*
      FROM avaliacoes av2
      WHERE av2.id = e.avaliacao_id
         OR (e.moto_avaliacao_id IS NOT NULL AND av2.moto_avaliacao_id = e.moto_avaliacao_id)
      ORDER BY
        CASE WHEN av2.id = e.avaliacao_id THEN 0 ELSE 1 END,
        av2.updated_at DESC NULLS LAST,
        av2.created_at DESC NULLS LAST
      LIMIT 1
    ) av ON true
  ...
  LOOP
    DECLARE
      v_tipo text := COALESCE(rec.tipo, 'propria');
      ...
      v_valor_fechamento numeric := COALESCE(
        NULLIF(rec.valor_fechamento, 0),
        NULLIF(rec.consignante_valor_fechamento, 0),
        NULLIF(rec.contrato_valor_fechamento, 0),
        CASE WHEN v_tipo = 'consignada' THEN NULLIF(rec.avaliacao_consignacao, 0) ELSE NULLIF(rec.avaliacao_compra, 0) END,
        0
      );
    ...
    BEGIN
      ...
      JOIN contratos_consignante cc2 ON cc2.id = co.contrato_consignante_id
      WHERE cc2.atendimento_id = rec.atendimento_id AND lower(co.responsavel) = 'loja';
