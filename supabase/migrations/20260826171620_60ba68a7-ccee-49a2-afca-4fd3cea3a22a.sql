-- =====================================================================
-- Renomeia atendimentos -> atendimentos_motos e extrai dados de cliente
-- para clientes_fornecedores (+ documentos + enderecos), referenciados
-- via atendimentos_motos.cliente_id.
--
-- Nao e necessario reescrever RLS em outras tabelas: as clausulas de
-- politica sao compiladas por OID, entao o rename automaticamente
-- continua valendo. So funcoes LANGUAGE plpgsql (corpo em texto puro)
-- e as politicas de storage precisam de edicao manual.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1) Rename
-- ---------------------------------------------------------------------
alter table public.atendimentos rename to atendimentos_motos;

-- ---------------------------------------------------------------------
-- 2) ramos_atividade (nao especificada pelo usuario; FK obrigatoria de
--    clientes_fornecedores.ramo)
-- ---------------------------------------------------------------------
create table public.ramos_atividade (
  id uuid primary key default gen_random_uuid(),
  nome text not null unique,
  created_at timestamptz not null default now()
);

alter table public.ramos_atividade enable row level security;

create policy "Autenticados veem ramos_atividade" on public.ramos_atividade for select to authenticated using (true);
create policy "Master gerencia ramos_atividade" on public.ramos_atividade for all to authenticated
  using (public.has_app_role(auth.uid(), 'master')) with check (public.has_app_role(auth.uid(), 'master'));

-- ---------------------------------------------------------------------
-- 3) clientes_fornecedores / documentos / enderecos (DDL do usuario)
-- ---------------------------------------------------------------------
create table public.clientes_fornecedores (
  ...
  cpf_cnpj text null,
  tipo_pessoa text not null default 'fisica'::text,
  tipo_cadastro text not null default 'cliente'::text,
  telefone text null,
  telefone_comercial text null,
  email text null,
  email_nf text null,
  ...
  validado_receita boolean null default false,
  data_validacao_receita timestamp with time zone null,
  origem_cadastro text null,
  status text null default 'ativo'::text,
  inscricao_estadual text null,
  inscricao_municipal text null,
  isento_inscricao_estadual boolean null default false,
  contribuinte_icms boolean not null default false,
  consumidor_final boolean not null default true,
  banco text null,
  agencia text null,
  conta text null,
  digito_conta text null,
  tipo_conta text null,
  chave_pix text null,
  favorecido text null,
  cpf_cnpj_favorecido text null,
  finalidade_cadastro text null,
  aceite_politica_privacidade boolean not null default true,
  data_aceite_politica_privacidade timestamp with time zone null default now(),
  ...
  constraint clientes_fornecedores_ramo_fkey foreign key (ramo) references ramos_atividade (id)
);

create index if not exists idx_cf_nome on public.clientes_fornecedores using btree (nome_razao_social);
create index if not exists idx_cf_status on public.clientes_fornecedores using btree (status) where (deleted_at is null);
create index if not exists idx_cf_tipo on public.clientes_fornecedores using btree (tipo_cadastro) where (deleted_at is null);
create index if not exists idx_cf_ramo on public.clientes_fornecedores using btree (ramo);
create index if not exists idx_cf_cpf_cnpj on public.clientes_fornecedores using btree (cpf_cnpj) where (deleted_at is null);
create index if not exists idx_cf_telefone on public.clientes_fornecedores using btree (telefone) where (deleted_at is null);

create trigger trg_cf_updated before update on public.clientes_fornecedores for each row execute function public.set_updated_at();

alter table public.clientes_fornecedores enable row level security;
create policy "Autenticados veem clientes_fornecedores" on public.clientes_fornecedores for select to authenticated using (true);
create policy "Autenticados criam clientes_fornecedores" on public.clientes_fornecedores for insert to authenticated with check (true);
create policy "Autenticados atualizam clientes_fornecedores" on public.clientes_fornecedores for update to authenticated using (true);
create policy "Master deleta clientes_fornecedores" on public.clientes_fornecedores for delete to authenticated using (public.has_app_role(auth.uid(), 'master'));

create table public.clientes_fornecedores_documentos (
  id uuid not null default gen_random_uuid (),
  cliente_fornecedor_id uuid not null,
  tipo_documento text not null,
  arquivo_url text not null,
  created_at timestamp with time zone not null default now(),
  constraint clientes_fornecedores_documentos_pkey primary key (id),
  constraint clientes_fornecedores_documentos_cliente_fornecedor_id_fkey foreign key (cliente_fornecedor_id) references clientes_fornecedores (id) on delete cascade
);

create index if not exists idx_cfd_cf on public.clientes_fornecedores_documentos using btree (cliente_fornecedor_id);

alter table public.clientes_fornecedores_documentos enable row level security;
create policy "Autenticados veem clientes_fornecedores_documentos" on public.clientes_fornecedores_documentos for select to authenticated using (true);
create policy "Autenticados criam clientes_fornecedores_documentos" on public.clientes_fornecedores_documentos for insert to authenticated with check (true);
create policy "Autenticados atualizam clientes_fornecedores_documentos" on public.clientes_fornecedores_documentos for update to authenticated using (true);
create policy "Master deleta clientes_fornecedores_documentos" on public.clientes_fornecedores_documentos for delete to authenticated using (public.has_app_role(auth.uid(), 'master'));

create table public.clientes_fornecedores_enderecos (
  id uuid not null default gen_random_uuid (),
  cliente_fornecedor_id uuid not null,
  tipo text not null default 'fiscal'::text,
  cep text null,
  ...
  created_at timestamp with time zone not null default now(),
  updated_at timestamp with time zone not null default now(),
  constraint clientes_fornecedores_enderecos_pkey primary key (id),
  constraint clientes_fornecedores_enderecos_cliente_fornecedor_id_fkey foreign key (cliente_fornecedor_id) references clientes_fornecedores (id) on delete cascade
);

create index if not exists idx_cfe_cf on public.clientes_fornecedores_enderecos using btree (cliente_fornecedor_id);

create trigger trg_cfe_updated before update on public.clientes_fornecedores_enderecos for each row execute function public.set_updated_at();

alter table public.clientes_fornecedores_enderecos enable row level security;
create policy "Autenticados veem clientes_fornecedores_enderecos" on public.clientes_fornecedores_enderecos for select to authenticated using (true);
create policy "Autenticados criam clientes_fornecedores_enderecos" on public.clientes_fornecedores_enderecos for insert to authenticated with check (true);
create policy "Autenticados atualizam clientes_fornecedores_enderecos" on public.clientes_fornecedores_enderecos for update to authenticated using (true);
create policy "Master deleta clientes_fornecedores_enderecos" on public.clientes_fornecedores_enderecos for delete to authenticated using (public.has_app_role(auth.uid(), 'master'));

-- ---------------------------------------------------------------------
-- 4) atendimentos_motos: adiciona cliente_id, remove colunas de cliente
--    (tabela vazia hoje - sem necessidade de backfill)
-- ---------------------------------------------------------------------
alter table public.atendimentos_motos
  add column cliente_id uuid references public.clientes_fornecedores(id);
...
-- ---------------------------------------------------------------------
-- 5) Funcoes LANGUAGE plpgsql que citam "atendimentos" no corpo (texto
--    puro, nao segue o rename sozinho) - recriar apontando para
--    atendimentos_motos. Nenhuma delas referencia colunas de cliente,
--    exceto relatorio_showroom_sinais/vendidas (nome_cliente), tratadas
--    com join para clientes_fornecedores.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      FROM avaliacoes av JOIN atendimentos_motos a ON a.id = av.atendimento_id
...
BEGIN
  ...
  WHILE v_start <= v_now LOOP
    ...
    BEGIN
      SELECT count(*) INTO v_avaliacoes FROM avaliacoes av JOIN atendimentos_motos a ON a.id = av.atendimento_id
...
BEGIN
  SELECT count(*) INTO v_qtd_atendimentos FROM atendimentos_motos a
  ...
  SELECT count(*) INTO v_qtd_vendas FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id
  ...
  SELECT count(*) INTO v_qtd_sinais FROM atendimentos_motos a WHERE a.situacao = 'sinal' AND (_loja = 'todos' OR norm_loja(a.loja) = _loja OR a.loja = _loja);
  FOR rec IN SELECT a.id as atend_id, a.valor_venda as atend_valor_venda, e.id as estoque_id, e.preco as estoque_preco, e.valor_venda as estoque_valor_venda, e.tipo as estoque_tipo, av.id as avaliacao_id, av.quanto_vende, av.valor_fechamento
    FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id LEFT JOIN avaliacoes av ON av.id = e.avaliacao_id
    WHERE a.situacao = 'vendido' AND (_loja = 'todos' OR norm_loja(a.loja) = _loja OR a.loja = _loja)
      AND a.data_venda IS NOT NULL AND (_tipo = 'todos' OR COALESCE(e.tipo, 'propria') = _tipo)
      AND (_date_from IS NULL OR a.data_venda >= _date_from) AND (_date_to IS NULL OR a.data_venda <= _date_to)
  LOOP
...
BEGIN
  WHILE v_start <= v_now LOOP
    ...
    BEGIN
      SELECT count(*) INTO v_atend FROM atendimentos_motos a
      WHERE (_loja = 'todos' OR norm_loja(a.loja) = _loja OR a.loja = _loja)
        AND a.created_at >= v_cycle_start AND a.created_at <= v_cycle_end;

      SELECT count(*) INTO v_vendas FROM atendimentos_motos a
...
BEGIN
  FOR v IN SELECT ur.user_id, ur.nome FROM user_roles ur LOOP
    ...
    BEGIN
      SELECT count(*) INTO v_atend FROM atendimentos_motos a WHERE a.vendedor_id = v.user_id AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND (_date_from IS NULL OR a.created_at >= _date_from) AND (_date_to IS NULL OR a.created_at <= _date_to);
      SELECT count(*) INTO v_vendas FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.vendedor_id=v.user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.data_venda IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND (_date_from IS NULL OR a.data_venda >= _date_from) AND (_date_to IS NULL OR a.data_venda <= _date_to);
      SELECT count(*) INTO v_sinais FROM atendimentos_motos a WHERE a.vendedor_id=v.user_id AND a.situacao='sinal' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja);
      SELECT COALESCE(SUM(COALESCE(e.preco, a.valor_venda, 0)),0) INTO v_faturamento FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.vendedor_id=v.user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.data_venda IS NOT NULL AND (_tipo='todos' OR COALESCE(e.tipo,'propria')=_tipo) AND (_date_from IS NULL OR a.data_venda >= _date_from) AND (_date_to IS NULL OR a.data_venda <= _date_to);
...
BEGIN
  FOR v IN SELECT ur.user_id, ur.nome FROM user_roles ur LOOP
    ...
    BEGIN
      SELECT count(*) INTO v_atend FROM atendimentos_motos a WHERE a.vendedor_id=v.user_id AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND (_date_from IS NULL OR a.created_at >= _date_from) AND (_date_to IS NULL OR a.created_at <= _date_to);
      SELECT count(*) INTO v_vendas FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id = a.id WHERE a.vendedor_id=v.user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.data_venda IS NOT NULL AND (_date_from IS NULL OR a.data_venda >= _date_from) AND (_date_to IS NULL OR a.data_venda <= _date_to);
      SELECT count(*) INTO v_sinais FROM atendimentos_motos a WHERE a.vendedor_id=v.user_id AND a.situacao='sinal' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja);
      IF v_atend>0 OR v_vendas>0 OR v_sinais>0 THEN
...
DECLARE v_atend bigint; v_vendas bigint; v_sinais bigint;
BEGIN
  SELECT count(*) INTO v_atend FROM atendimentos_motos a WHERE a.vendedor_id=_user_id AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND (_date_from IS NULL OR a.created_at >= _date_from) AND (_date_to IS NULL OR a.created_at <= _date_to);
  SELECT count(*) INTO v_vendas FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id=a.id WHERE a.vendedor_id=_user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.data_venda IS NOT NULL AND (_date_from IS NULL OR a.data_venda >= _date_from) AND (_date_to IS NULL OR a.data_venda <= _date_to);
  SELECT count(*) INTO v_sinais FROM atendimentos_motos a WHERE a.vendedor_id=_user_id AND a.situacao='sinal' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja);
...
BEGIN
  WHILE v_start <= v_now LOOP
    ...
    BEGIN
      SELECT count(*) INTO v_atend FROM atendimentos_motos a WHERE a.vendedor_id=_user_id AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.created_at >= v_cs AND a.created_at <= v_ce;
      SELECT count(*) INTO v_vendas FROM atendimentos_motos a LEFT JOIN estoque e ON e.atendimento_venda_id=a.id WHERE a.vendedor_id=_user_id AND a.situacao='vendido' AND (_loja='todos' OR norm_loja(a.loja)=_loja OR a.loja=_loja) AND a.data_venda IS NOT NULL AND a.data_venda >= v_cs AND a.data_venda <= v_ce;
...
-- relatorio_showroom_sinais / relatorio_showroom_vendidas: alem do rename,
-- ganham join para clientes_fornecedores (nome_cliente saiu de atendimentos)
...
BEGIN
  FOR rec IN
    ...
    LEFT JOIN LATERAL (SELECT e2.* FROM estoque e2 WHERE e2.atendimento_venda_id = a.id OR (mi.estoque_moto_id IS NOT NULL AND e2.id::text = mi.estoque_moto_id)
      ORDER BY CASE WHEN mi.estoque_moto_id IS NOT NULL AND e2.id::text = mi.estoque_moto_id THEN 0 WHEN e2.atendimento_venda_id = a.id THEN 1 ELSE 2 END, e2.updated_at DESC NULLS LAST, e2.created_at DESC NULLS LAST LIMIT 1) e ON true
    LEFT JOIN LATERAL (SELECT av2.* FROM avaliacoes av2 WHERE av2.id = e.avaliacao_id OR (e.moto_avaliacao_id IS NOT NULL AND av2.moto_avaliacao_id = e.moto_avaliacao_id)
      ORDER BY CASE WHEN av2.id = e.avaliacao_id THEN 0 ELSE 1 END, av2.updated_at DESC NULLS LAST, av2.created_at DESC NULLS LAST LIMIT 1) av ON true
...
-- ---------------------------------------------------------------------
-- 6) storage.objects: caminho de CNH passa a ser docs/{cliente_id}/...
-- ---------------------------------------------------------------------
    ...
    and (
      (
        ...
        and exists (
          ...
          where a.cliente_id::text = (storage.foldername(name))[2]
            and (a.vendedor_id = auth.uid() or public.has_master_or_gerente_empresa(auth.uid(), a.loja))
        )
      )
      or exists (
        select 1 from public.motos_avaliacao ma join public.atendimentos_motos a on a.id = ma.atendimento_id
